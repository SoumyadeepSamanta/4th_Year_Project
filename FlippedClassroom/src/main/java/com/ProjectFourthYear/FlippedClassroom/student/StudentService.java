...
import com.ProjectFourthYear.FlippedClassroom.Submissions.Submissions;
...
public class StudentService {
    ...
    public void save(MultipartFile file) {
        try {
          List<Student> students = ExcelHelperStudent.excelToTutorials(file.getInputStream());
    ...
    public List<Subject_Student_DTO> getSubjectsAndTeacherByStudentId(String studentId) {
        ...
        while (iterator.hasNext()) {
            ...
            Subject_Student_DTO dto = new Subject_Student_DTO(
                ...
                (String) row[3],        // teacher name
                (String) row[4]     //student id
            );
    ...
    public List<StudentDTO> checkPassword(String student_id){
        List<Object[]> results=studentRepository.checkPassword(student_id);

        return results.stream()
                .map(row -> new StudentDTO(
                    (String) row[0]
                ))
                .collect(Collectors.toList());
    }   
    ...
    // public List<SubmissionDTO> getSubmissions(String department,String subid) {
    //     List<Object[]> results = submissionRepository.findsubmission(subid,department);
    ...
    public Submissions addSubmissions(Long assignment_id,String sub_id,Submissions submission){
